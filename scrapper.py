        ...
        if response.status_code == 200:
            ...
            for player in server_data['Data']['players']:
                ...
                if not check_if_player_exists(filename, player, added_players):
                    ...
                    print( Fore.CYAN + f'[NEW]' + Style.RESET_ALL + f' {player["name"]} a été ajouté à la db !')
                    added_players.append(player_identifiers)

            print('\n' + Fore.MAGENTA + '\n[Info]' + Style.RESET_ALL + f' Serveur ID : {server_id}' + Fore.MAGENTA + '\n[Info]' + Style.RESET_ALL + f' SERVEUR : {filename}' + '\n')

        else:
            print(Fore.RED + f'\n[Erreur]' + Style.RESET_ALL + f" Message d'erreur ({server_id}: {response.status_code})\n")
